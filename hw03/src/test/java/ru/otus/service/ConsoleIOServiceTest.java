...
class ConsoleIOServiceTest {
    ...
    void shouldPrintAMessageWithoutArgumentsInEnglish() {
        ...
        when(messageSource.getMessage(TEXT_TO_PRINT_WITHOUT_ARGS, new String[]{TEXT_TO_ARG_EN}, new Locale("en")))
                .thenReturn(TEXT_TO_READ_WITHOUT_ARGS_EN);
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        IOService ioService = new ConsoleIOService(messageSource, props, System.in, new PrintStream(bos));
        ioService.out(TEXT_TO_PRINT_WITHOUT_ARGS);
    ...
    void shouldPrintAMessageWithArgumentsInRussian() {
        when(props.getLocale()).thenReturn(new Locale("ru_RU"));
        when(messageSource.getMessage(TEXT_TO_PRINT_WITH_ARGS, new String[]{TEXT_TO_ARG_RU}, new Locale("ru_RU")))
                .thenReturn(TEXT_TO_READ_WITH_ARGS_RU);
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        IOService ioService = new ConsoleIOService(messageSource, props, System.in, new PrintStream(bos));
    ...
    void shouldPrintAMessageWithArgumentsInEnglish() {
        when(props.getLocale()).thenReturn(new Locale("en"));
        when(messageSource.getMessage(TEXT_TO_PRINT_WITH_ARGS, new String[]{TEXT_TO_ARG_EN}, new Locale("en")))
            .thenReturn(TEXT_TO_READ_WITH_ARGS_EN);
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        IOService ioService = new ConsoleIOService(messageSource, props, System.in, new PrintStream(bos));
